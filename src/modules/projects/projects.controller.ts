...
export class ProjectsController {
    ...
    @ApiCommonErrorResponse(ErrorCode.INVALID_INVITE_CODE, '유효하지 않은 초대코드입니다.', HttpStatus.BAD_REQUEST)
    @ApiCommonErrorResponse(ErrorCode.PROJECT_ALREADY_COMPLETED, '이미 완료된 프로젝트입니다', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.EXPIRED_INVITE_CODE, '유효기간이 지난 url입니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.ALREDY_JOIN, '이미 프로젝트에 참여하였습니다.', HttpStatus.FORBIDDEN)
    @Get('/join/validate')
    ...
    @ApiCommonErrorResponse(ErrorCode.PROJECT_NOT_FOUND, '프로젝트를 찾을 수 없습니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(
    ...
    @ApiCommonResponse(AllProjectResponseDto)
    @ApiCommonErrorResponse(ErrorCode.PROJECT_NOT_FOUND, '프로젝트를 찾을 수 없습니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.FORBIDDEN_USER_FOR_UPDATE, '해당 항목을 수정할 권한이 없습니다.', HttpStatus.FORBIDDEN)
    @Transactional()
    @Patch('/:projectId')
    async updateProject(
        @Req() req: TransactionalRequest,
    ...
    @ApiCommonErrorResponse(ErrorCode.PROJECT_NOT_FOUND, '프로젝트를 찾을 수 없습니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.POSTS_EXCEEDED, '포스트잇은 10개까지 생성될 수 있습니다.', HttpStatus.FORBIDDEN)
    ...
    @ApiCommonErrorResponse(ErrorCode.PROJECT_NOT_FOUND, '프로젝트를 찾을 수 없습니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.POST_NOT_FOUND, '포스트잇을 찾을 수 없습니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.NOT_POST_AUTHOR, '포스트잇 작성자만 삭제할 수 있습니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(
    ...
    @ApiCommonErrorResponse(ErrorCode.ASSIGNEE_NOT_MEMBER, '해당 사람은 프로젝트 멤버가 아닙니다.', HttpStatus.FORBIDDEN)
    @ApiCommonErrorResponse(ErrorCode.ALREDY_LEADER, '이미 팀장인 사용자입니다.', HttpStatus.FORBIDDEN)
    @Transactional()
    ...
    @ApiCommonErrorResponse(ErrorCode.FORBIDDEN_USER_FOR_PROJECT,
            '해당 프로젝트에 접근 권한이 없습니다.',
            HttpStatus.FORBIDDEN)
    //TODO: 응답 명세
